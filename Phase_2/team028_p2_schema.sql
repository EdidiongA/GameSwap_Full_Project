...
CREATE TABLE `Item` (
  ...
  `condition` 		VARCHAR(50) 	NOT NULL	CHECK (`condition` IN ('Mint', 'LikeNew', 'LightlyUsed', 'ModeratelyUsed', 'HeavilyUsed', 'Damaged')),
  `type`			VARCHAR(50)		NOT NULL	CHECK (`type` IN ('BoardGame', 'CardGame', 'VideoGame', 'ComputerGame', 'JigsawPuzzle')),
  PRIMARY KEY (`item_id`),
  FOREIGN KEY (`email`) REFERENCES `User` (`email`)
);
...
CREATE TABLE `VideoGame` (
  `item_id`			BIGINT 			NOT NULL,
  `platform_name` 	VARCHAR(50) 	NOT NULL,
  `media` 			VARCHAR(50)		NOT NULL	CHECK (`media` IN ('Cartridge', 'GameCard', 'OpticalDisc')),
  PRIMARY KEY (`item_id`),
  FOREIGN KEY (`item_id`) REFERENCES `Item` (`item_id`),
